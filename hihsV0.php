...
function n(t){var n=prompt('Name for new '+(t==='f'?'file':'directory')+':');if(n){var r=new XMLHttpRequest();r.open('POST','',true);r.setRequestHeader('Content-Type','application/x-www-form-urlencoded');r.send('n='+encodeURIComponent(n)+'&t='+t);}return false;}
function u(f){var fd=new FormData();fd.append('f',f.files[0]);var r=new XMLHttpRequest();r.open('POST','',true);r.onload=function(){document.getElementById('o').textContent=this.responseText;};r.send(fd);}
</script>
</body>
</html>
